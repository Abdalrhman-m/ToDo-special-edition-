...
function showAlert(isChecked) {
    if (listco.children.length === 0) {
        alert("that's it for today nasnosty 🐥❤️ !!  اشطر نسنووسة  ");
    } else if (isChecked) {
        alert("vaammooss , keep going ya nosnostyyy 🐥❤️🔥");
    } else {
        alert("vaammooss , keep going ya nosnostyyy 🐥❤️🔥");
    }
}
...
function store() {
    localStorage.setItem('taskss', listco.innerHTML);
}

function display() {
    if (localStorage.getItem('taskss')) {
        listco.innerHTML = localStorage.getItem('taskss');
    }
    document.getElementById("total-tasks").textContent = totalTasks;
    document.getElementById("completed-tasks").textContent = completedTasks;
}
